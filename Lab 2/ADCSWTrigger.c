...
void ADC0_InitSWTriggerSeq3_Ch9(void){ 
 SYSCTL_RCGCADC_R |= 0x0001;   // 7) activate ADC0 
  ...
  GPIO_PORTE_DIR_R &= ~0x06;      // 2) make PE1 and PE2 input
  GPIO_PORTE_AFSEL_R |= 0x06;     // 3) enable alternate function on PE4
  GPIO_PORTE_DEN_R &= ~0x06;      // 4) disable digital I/O on PE4
  GPIO_PORTE_AMSEL_R |= 0x06;     // 5) enable analog functionality on PE4
...
//  while((SYSCTL_PRADC_R&0x0001) != 0x0001){};    // good code, but not yet implemented in simulator
  ...
  ADC0_SSMUX3_R += 1;             // Y. pe2   set channel
	ADC0_SSMUX3_R += 2;             // X. pe1   set channel
