...
void DAC_Init(uint16_t data){
  ...
  SYSCTL_RCGCGPIO_R |= 0x01;      // activate port A
  while((SYSCTL_PRGPIO_R&0x01) == 0){};// ready?
  GPIO_PORTA_AFSEL_R |= 0x2C;     // enable alt funct on PA2,5, regular GPIO on PA3
  ...
  SSI2_CPSR_R = 0x02;             // 8 MHz SSIClk //This effing register 
